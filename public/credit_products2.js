...
function put_order() {
    let checks = $("#creditor_table").find(".check")
    prod_rows = []
    prods = []
    checks.each(function (index, ele) {
        if (ele.checked) {
            ...
            prod_rows.push({ id: prod_id, quantity: prod_quant, name: prod_name, discount: prod_discount, mrp: prod_mrp, sp: prod_sp, cp: prod_cp, total_cost: prod_sp * prod_quant })
        }
    })
    console.log(prod_rows)
    $.post("/credit_products3", { products: JSON.stringify(prods) }, function (data) {
        console.log(data)
        $("#title").html("Confirm your credit order...")
...
function confirm_order() {
    window.location.href = "/credit_products_final";
}
